...
public abstract class ImageWorker {
	 ...
	 */
	public void loadImage(Object data, ImageView imageView, boolean isOnlineLoad) {
		if (data == null) {
			return;
		}
		...
		if (value != null) {
			...
			final BitmapWorkerTask task = new BitmapWorkerTask(imageView, isOnlineLoad);
	...
	private class BitmapWorkerTask extends AsyncTask<Object, Void, BitmapDrawable> {
		...
		private final boolean mIsOnlineLoad;

		public BitmapWorkerTask(ImageView imageView, boolean isOnlineLoad) {
			imageViewReference = new WeakReference<ImageView>(imageView);
			mIsOnlineLoad = isOnlineLoad;
		}
		...
		protected BitmapDrawable doInBackground(Object... params) {
			...
			if (bitmap == null && !isCancelled() && getAttachedImageView() != null && !mExitTasksEarly) {
				if (mIsOnlineLoad) {
					...
					try {
						bitmap = BitmapFactory.decodeStream(mResources.getAssets().open((String) params[0]));
					} catch (IOException e) {
						e.printStackTrace();
					}
				}
